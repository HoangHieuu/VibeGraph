...
| US-601 | Packed npm CLI bundles the dashboard/backend, bootstraps isolated Python, and launches outside the monorepo | yes | yes | yes | yes | in_progress | `pnpm check`; `pnpm package`; fresh tarball install and launch passed on macOS with 2-file graph; packaged browser QA passed with clean console; Ubuntu/Windows CI workflow added, with first remote run and public registry publication still pending |
